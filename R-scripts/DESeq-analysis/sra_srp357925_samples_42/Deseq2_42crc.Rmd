---
title: "Differential Gene Expression of 42 colorectal cancer samples in R"
...
# Data preparation
## Load dataset
After completing the `nf-core/rnaseq` pipeline using Salmon pseudoalignment for the analysis of 42 colorectal samples from SRA study SRP357925 will obtain the `salmon.merged.gene.SummarizedExperiment.rds`
...
- Go to [SRA study SRP357925](https://trace.ncbi.nlm.nih.gov/Traces/index.html?view=study&acc=SRP357925)
...
As follows, a metadata `.csv` file was created using information from [BioProject: PRJNA802883](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA802883). Here are the first lines of the created `sraruntable_srp357925_crc42_metadata.csv` file. 
...
```{r}
# Load files
df_metadata <- read.csv("sraruntable_srp357925_crc42_metadata.csv", header=TRUE, check.names = FALSE)
...
ggplot(df, aes(PC1, PC2)) + 
  ...
  scale_color_manual(values = c("normal_eocrc" = "green", 
                                "tumor_eocrc" = "red"))
...
write.table(resSig, file = "DESeq2_sig_genes_normal_vs_cancer_42_eocrc.tab", sep = "\t", quote = FALSE , row.names = FALSE)
```
...
plotCounts(dds_DGE, gene = "ENSG00000237550.8", intgroup=c("condition"))

# Individual gene raw counts using ggplot2
geneCounts <- plotCounts(dds_DGE, 
                         gene = "ENSG00000237550.8", 
...
res <- lfcShrink(dds_DGE, coef="condition_tumor_eocrc_vs_normal_eocrc", type="apeglm")
...
writeLines(table_tex, "tables/Top_DEG_genes_volcano_plot_eocrc.tex")
```
...
# file = "DESeq2_sig_genes_normal_vs_cancer_42_eocrc.tab"
head(resSig)
```
...
write.table(resSig_df, file=("DESeq2_sig_genes_normal_vs_cancer_42_eocrc_annotated.tab"), row.names = F,col.names = TRUE, sep="\t")
```
...
# Transform into a matrix and use vst transformation
...
tumor_vs_normal_42_dds <- results(dds_DGE,
                           contrast = c("condition", "tumor_eocrc", "normal_eocrc"))
...
DE_tumor_vs_normal_42 <- subset(tumor_vs_normal_42_dds,
                  padj < 0.05 & abs(log2FoldChange) >= 1.0)

# Order significant genes by p-adjusted value 
DE_tumor_vs_normal_42 <- DE_tumor_vs_normal_42[ order(DE_tumor_vs_normal_42$padj) ,  ]
head(DE_tumor_vs_normal_42[ , -1], 5)

# Save results
write.csv(DE_tumor_vs_normal_42, "DE_tumor_vs_normal_42.csv")
```
